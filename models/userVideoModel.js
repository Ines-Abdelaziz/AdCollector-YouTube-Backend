...
class UserVideoModel {
    static async addUserVideo(userVideoData) {
        ...
        try {
            const result = await pool.query('INSERT INTO watch_history (user_id,video_id) VALUES ($1, $2) RETURNING *', [user_id, video_id]);
    ...
    static async getUserVideoByAllColumns(userVideoData){
        const { user_id ,video_id} = userVideoData;
        try {
            const result = await pool.query('SELECT * FROM watch_history WHERE user_id = $1 AND video_id = $2', [user_id, video_id]);
            return result.rows[0];
        } catch (error) {
